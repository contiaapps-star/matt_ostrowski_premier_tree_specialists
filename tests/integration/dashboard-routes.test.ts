...
describe('GET /dashboard', () => {
  ...
  it('returns 200 with HTML, header label, and at least one lead row', async () => {
    const db = getDb();
    insertLead(db, {
    ...
    expect(html).toContain('data-testid="lead-row"');
  });
  ...
  it('shows an empty state when no leads match', async () => {
    ...
    expect(res.status).toBe(200);
    const html = await res.text();
    expect(html).toContain('data-testid="empty-state-dashboard"');
  });
  ...
  it('ignores unknown source/status filter values without crashing', async () => {
    const db = getDb();
    insertLead(db, { customerName: 'Anyone' });
    const app = createApp();
    const res = await app.request('/dashboard?source=hacker&status=xyz');
    expect(res.status).toBe(200);
    const html = await res.text();
...
describe('GET /dashboard/leads-table', () => {
  beforeEach(() => setupFreshDb());
  afterEach(() => teardownDb());

  it('returns only the table partial (no <head>, no nav)', async () => {
    const db = getDb();
    insertLead(db, { customerName: 'Partial Test' });
    const app = createApp();
    const res = await app.request('/dashboard/leads-table');
    expect(res.status).toBe(200);
    const html = await res.text();
    expect(html).toContain('Partial Test');
    expect(html).toContain('data-testid="leads-table-container"');
    // Layout markers should be ABSENT in a partial.
    expect(html).not.toContain('<!DOCTYPE');
    expect(html).not.toContain('<html');
    expect(html).not.toContain('Premier Tree Specialists');
  });

  it('returns an empty-state partial when no leads match', async () => {
    const app = createApp();
    const res = await app.request('/dashboard/leads-table?source=website_form');
    expect(res.status).toBe(200);
    const html = await res.text();
    expect(html).toContain('data-testid="empty-state-dashboard"');
    expect(html).not.toContain('<!DOCTYPE');
  ...
  it('respects source filter on partial endpoint', async () => {
    ...
    const app = createApp();
    const res = await app.request('/dashboard/leads-table?source=google_lsa_email');
    const html = await res.text();

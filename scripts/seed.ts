...
function zips(county: string, region: 'northeast_ohio' | 'central_ohio', zs: string[]): NewZipRow[] {
  return zs.map((zip) => ({ zip, county, region }));
}

const FAQ_ROWS: Omit<NewFaqEntry, 'id'>[] = [
  {
    category: 'oak_season',
...
function buildLeadSpecs(): SeedLeadSpec[] {
  const baseTime = Date.UTC(2026, 3, 26, 12, 0, 0); // 2026-04-26T12:00:00Z
  const minute = 60_000;
